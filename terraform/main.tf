...
resource "aws_lambda_function" "dynamodb_to_bigquery" {
  filename         = "/home/ec2-user/cloudmart/cloudmart-app/backend/src/lambda/addToBigQuery/dynamodb_to_bigquery.zip"
  function_name    = "cloudmart-dynamodb-to-bigquery"
  role             = aws_iam_role.lambda_role.arn
  handler          = "index.handler"
  runtime          = "nodejs20.x"
  source_code_hash = filebase64sha256("/home/ec2-user/cloudmart/cloudmart-app/backend/src/lambda/addToBigQuery/dynamodb_to_bigquery.zip")


  environment {
    variables = {
      GOOGLE_CLOUD_PROJECT_ID        = "optical-aviary-446420-i1"
